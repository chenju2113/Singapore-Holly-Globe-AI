      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
        ...
        <div className="lg:col-span-6 space-y-6">
          <span className="text-xs font-mono-code font-bold text-[#0056c5] uppercase tracking-wider bg-[#eff4ff] px-3 py-1 rounded border border-[#d9e2ff]">
            {language === 'zh' ? '关于我们' : language === 'ja' ? '私たちについて' : language === 'ms' ? 'TENTANG KAMI' : language === 'vi' ? 'VỀ CHÚNG TÔI' : 'WHO WE ARE'}
          </span>

          <h2 className="text-3xl sm:text-4xl font-extrabold text-[#0b1c30] tracking-tight">
            {language === 'zh' ? '新加坡 · 中国 AI 能见度战略伙伴' : language === 'ja' ? 'シンガポール · 中国AI検索可視化戦略パートナー' : language === 'ms' ? 'Singapura · Rakan Keterlihatan AI China' : language === 'vi' ? 'Singapore · Đối tác hiển thị AI Trung Quốc' : 'Singapore · China AI Visibility Partner'}
          </h2>
          ...
          <p className="text-[#424654] text-base sm:text-lg leading-relaxed">
            {language === 'zh'
              ...
              : language === 'ja'
              ? 'HollyGlobe シンガポールは、シンガポールおよび東南アジア・グローバル企業が中国の主要AI検索エコシステムにおいて確固たる可視性、ナレッジグラフ、LLM推奨引用を獲得するためのオンライン特化型B2B戦略アドバイザリーです。'
              : language === 'ms'
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div className="p-4 bg-white border border-[#c2c6d7] rounded-lg">
              ...
              <div className="text-xs font-bold text-[#0b1c30] mt-1">
                {language === 'zh' ? '线上咨询顾问' : language === 'ja' ? 'オンライン特化アドバイザリー' : language === 'ms' ? 'Konsultasi Dalam Talian' : language === 'vi' ? 'Tư vấn trực tuyến' : 'Online-First'}
              </div>
              <div className="text-[11px] text-[#727685]">
                {language === 'zh' ? '远程战略咨询与数字化服务' : language === 'ja' ? 'リモート戦略セッション＆デジタル支援' : language === 'ms' ? 'Konsultasi Maya & Penasihat Digital' : language === 'vi' ? 'Tư vấn từ xa & cố vấn kỹ thuật số' : 'Remote Consultation & Digital Advisory'}
              </div>
            </div>
            ...
            <div className="p-4 bg-white border border-[#c2c6d7] rounded-lg">
              ...
              <div className="text-xs font-bold text-[#0b1c30] mt-1">
                {language === 'zh' ? '新加坡总部基地' : language === 'ja' ? 'シンガポール拠点' : language === 'ms' ? 'Pangkalan Singapura' : language === 'vi' ? 'Trụ sở Singapore' : 'Singapore Base'}
              </div>
              <div className="text-[11px] text-[#727685]">
                {language === 'zh' ? '跨境 AI 能见度专家团队' : language === 'ja' ? '越境AI可視化専門エキスパート' : language === 'ms' ? 'Pakar Keterlihatan AI Rentas Sempadan' : language === 'vi' ? 'Chuyên gia hiển thị AI xuyên biên giới' : 'Cross-Border AI Visibility Specialists'}
              </div>
            </div>
          </div>
          ...
          <div className="p-8 bg-white border border-[#c2c6d7] rounded-2xl shadow-sm space-y-6">
            <div className="flex items-center justify-between border-b border-[#e2e8f0] pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold text-[#0b1c30] text-base">
                    {language === 'zh' ? '新加坡 B2B 线上咨询中心' : language === 'ja' ? 'シンガポール B2B オンライン相談デスク' : language === 'ms' ? 'Meja Penasihat B2B Dalam Talian SG' : language === 'vi' ? 'Bàn tư vấn B2B trực tuyến SG' : 'Online-First B2B Advisory Desk'}
                  </div>
                  <div className="text-xs text-[#727685]">
                    {language === 'zh' ? '新加坡 · 远程战略顾问对接' : language === 'ja' ? 'シンガポール · リモート戦略アドバイザリー' : language === 'ms' ? 'Singapura · Penglibatan Strategik Maya' : language === 'vi' ? 'Singapore · Kết nối chiến lược từ xa' : 'Singapore · Remote Strategic Engagement'}
                  </div>
                </div>
              </div>
              <span className="text-xs font-mono-code bg-emerald-50 text-emerald-700 font-bold px-2.5 py-1 rounded border border-emerald-200">
                {language === 'zh' ? '实时在线' : language === 'ja' ? '受付中' : 'ACTIVE'}
              </span>
            </div>
            ...
            <div className="space-y-3 text-sm text-[#424654]">
              <div className="flex items-start gap-3">
                ...
                <span>
                  {language === 'zh' ? '总部位于新加坡 · 全程远程支持东南亚区域企业' : language === 'ja' ? 'シンガポール拠点 · 地域・グローバル企業をオンラインで完全サポート' : language === 'ms' ? 'Berpusat di Singapura · Menyokong syarikat serantau secara maya' : language === 'vi' ? 'Trụ sở tại Singapore · Hỗ trợ doanh nghiệp khu vực từ xa' : 'Based in Singapore · Supporting regional enterprises remotely'}
                </span>
              </div>
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span>
                  {language === 'zh' ? '周一至周五: 09:00 - 18:00 (新加坡时间 / UTC+8)' : language === 'ja' ? '月曜〜金曜: 09:00 - 18:00 (SGT / 日本時間 10:00 - 19:00)' : 'Mon - Fri: 09:00 - 18:00 (SGT / UTC+8)'}
                </span>
              </div>
            </div>
            ...
            <div className="p-4 bg-[#f8f9ff] border border-[#d9e2ff] rounded-lg flex items-center justify-between text-xs">
              <span className="font-semibold text-[#0b1c30]">
                {language === 'zh' ? '跨境服务专线' : language === 'ja' ? '越境アドバイザリー対応' : language === 'ms' ? 'Talian Penasihat Rentas Sempadan' : language === 'vi' ? 'Đường dây tư vấn xuyên biên giới' : 'Cross-Border Advisory Line'}
              </span>
              <span className="font-mono-code text-[#0056c5] font-bold">
                {language === 'zh' ? '新加坡时间同步' : language === 'ja' ? 'UTC+8 リアルタイム同期' : 'UTC+8 Synchronized'}
              </span>
            </div>
          </div>
        </div>

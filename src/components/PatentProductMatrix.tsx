  ...
  return (
    <section id="product-matrix" className="py-10 px-6 max-w-[1280px] mx-auto border-t border-[#c2c6d7] bg-[#f8f9ff]/80 space-y-8 scroll-mt-20">
      ...
      <div className="space-y-4 text-center">
        <div className="space-y-1">
          <span className="text-xs font-mono-code font-bold text-[#727685] tracking-wide uppercase">
            {language === 'zh' ? '网信办监管备案与合规认证' : language === 'ms' ? 'PEMATUHAN ALGORITMA' : language === 'vi' ? 'TUÂN THỦ THUẬT TOÁN' : 'Compliance'}
          </span>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-[#0b1c30]">
            {language === 'zh' ? '中国网信办生成式 AI 算法备案' : language === 'ms' ? 'Pendaftaran Algoritma CAC' : language === 'vi' ? 'Đăng ký thuật toán CAC' : 'Algorithm Filing'}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
          ...
          <div className="p-4 sm:p-5 bg-white border border-[#e2e8f0] rounded-xl shadow-xs hover:border-[#0056c5] transition-all flex items-center gap-3.5 text-left">
            ...
            <div className="space-y-0.5">
              <h3 className="font-extrabold text-[#0b1c30] text-sm">
                reelsagent {language === 'zh' ? '文本生成算法' : 'Text Generation Algorithm'}
              </h3>
              <p className="text-xs font-mono-code text-[#727685]">
                {language === 'zh' ? '备案号' : 'Filing No.'} <span className="font-semibold text-[#0b1c30]">440105279266201250013</span>
              </p>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-5 bg-white border border-[#e2e8f0] rounded-xl shadow-xs hover:border-[#0056c5] transition-all flex items-center gap-3.5 text-left">
            ...
            <div className="space-y-0.5">
              <h3 className="font-extrabold text-[#0b1c30] text-sm">
                Hollyglobe {language === 'zh' ? '文本生成算法' : 'Text Generation Algorithm'}
              </h3>
              <p className="text-xs font-mono-code text-[#727685]">
                {language === 'zh' ? '备案号' : 'Filing No.'} <span className="font-semibold text-[#0b1c30]">440105284740101250017</span>
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-2xl sm:text-3xl font-extrabold text-[#0b1c30]">
          {language === 'zh' ? 'AI 智能营销矩阵与 Agent 解决方案' : language === 'ms' ? 'Matriks Produk Pemasaran AI' : language === 'vi' ? 'Ma trận sản phẩm tiếp thị AI' : 'AI Marketing Product Matrix'}
        </h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {PRODUCTS.map((prod) => {
            const desc1 = language === 'zh' ? prod.desc1Zh : language === 'ms' ? prod.desc1Ms : language === 'vi' ? prod.desc1Vi : prod.desc1En;
            const desc2 = language === 'zh' ? prod.desc2Zh : language === 'ms' ? prod.desc2Ms : language === 'vi' ? prod.desc2Vi : prod.desc2En;
            ...
            return (
              ...
              >
                ...
                <div className="pt-2 border-t border-[#e2e8f0]">
                  ...
                  >
                    <span>{language === 'zh' ? '咨询此方案' : language === 'ms' ? 'Tanya Ejen Ini' : language === 'vi' ? 'Tư vấn Agent này' : 'Enquire Agent'}</span>

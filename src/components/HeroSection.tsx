      ...
      <div className="max-w-4xl">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
          ...
          >
            ...
            <span>{language === 'zh' ? '免费获取中国 AI 能见度评估' : language === 'ms' ? 'Minta Ujian Keterlihatan AI China' : language === 'vi' ? 'Yêu cầu kiểm tra hiển thị AI Trung Quốc' : 'Request Your China AI Visibility Test'}</span>
          </button>
        </div>
        ...
        <div className="mt-8 flex items-center gap-6 text-xs text-[#727685] font-medium">
          <div className="flex items-center gap-1.5">
            ...
            <span>{language === 'zh' ? '新加坡线上咨询' : language === 'ms' ? 'Penasihat Dalam Talian SG' : language === 'vi' ? 'Tư vấn trực tuyến SG' : 'Singapore Online Advisory'}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="material-symbols-outlined text-base text-[#0056c5]">gavel</span>
            <span>{language === 'zh' ? '网信办 AI 算法备案合规' : language === 'ms' ? 'Pematuhan AI CAC China' : language === 'vi' ? 'Tuân thủ AI CAC Trung Quốc' : 'CAC AI Regulatory Compliance'}</span>
          </div>
          <div className="flex items-center gap-1.5 hidden md:flex">
            <span className="material-symbols-outlined text-base text-[#0056c5]">hub</span>
            <span>{language === 'zh' ? '跨境 Vector & RAG 准备度' : language === 'ms' ? 'Kesediaan Vector Rentas Sempadan' : language === 'vi' ? 'Sẵn sàng Vector xuyên biên giới' : 'Cross-Border Vector Readiness'}</span>
          </div>
        </div>
      </div>
    </section>
